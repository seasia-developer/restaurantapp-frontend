...
const SeachResult = () => {
  ...
  const searchDataRedux =
    useSelector((state: any) => state?.searchFilter?.data) ||
    getDataFromLocalStorage("searchDataRedux");
  
  // const searchData = useSelector((state) => state[searchFilterApi.reducerPath]);
  ...
  const handleNextSearch = async (name: any) => {
    if (name) {
      try {
        // const { data: nextData, error, isLoading } = useListByNameQuery(`${name}`);
        setSearchName(name);
      ...
      <div className="container">
        <div className={styles.searchResult}>
          <div className={`${styles.wsFilterSec} d-flex align-items-center`}>
            ...
            <div style={{position:"relative"}} onClick={()=> setShowModal(!showModal)} className={styles.wsFilterSec_P}>SORT
